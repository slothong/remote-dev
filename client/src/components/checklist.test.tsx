...
describe('go 명령어를 전송한 뒤 해당 항목은 disabled이고 go 버튼은 로딩중으로 표시한다', () => {
  it('Go 버튼 클릭 후 버튼이 disabled 되고 로딩 상태를 표시한다', async () => {
    // Mock WebSocket
    const mockSend = vi.fn();
    const mockWebSocket = {
      readyState: WebSocket.OPEN,
    ...
    // Get first Go button
    const goButtons = screen.getAllByTitle('Execute this task');
    const firstGoButton = goButtons[0] as HTMLButtonElement;

    // Verify button is initially enabled
    expect(firstGoButton.disabled).toBe(false);
    ...
    await waitFor(() => {
      expect(firstGoButton.disabled).toBe(true);
    });

    // Verify loading indicator is present (e.g., spinner or loading text)
    // The button should show loading state visually
    expect(firstGoButton.className).toContain('loading');
  });
});
