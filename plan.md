...
- [x] "연결 정보 기억하기" 체크박스를 해제하고 폼 submit 하면 로컬 스토리지에 있는 연결 정보를 제거한다.
- [x] plan.md 파일의 항목이 [-] 인 경우 go 버튼을 로딩 중으로 표시한다. 이를 구현하기 위해 plan.md 파일의 변경을 감지할 수 있어야 한다.
- [x] 모바일 화면에서 터미널 상단에 고정된 영역에 버튼 리스트가 표시된다. 위, 아래, 왼쪽, 오른쪽 화살표 버튼과 esc, enter 버튼이 포함된다. 각 버튼을 터치하면 해당 키 입력이 터미널로 전송된다.
- [x] 모바일 화면에서 터미널 헤더는 표시하지 않는다.
